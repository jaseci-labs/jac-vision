    ...
    try:
        ...
        for model in models:
            ...
            param_count = (
                ...
                else 7.0 if "7b" in model_name.lower() else None
            )
            if (
                not param_count
                or param_count < param_range_min
                or param_count > param_range_max
            ):
                continue
            ...
            if hardware_gpu_memory and gpu_memory > hardware_gpu_memory:
                continue
            pros = [
                "Strong in VQA" if "vqa" in tags else "",
                "Multilingual" if "multilingual" in tags else "",
            ]
            cons = [
                "High requirements" if param_count > 4.0 else "",
                "Less tested" if downloads < 1000 else "",
            ]
            filtered.append(
                {
                    "name": model_name,
...
def get_model_recommendations(
    task_type, param_range_min, param_range_max, hardware_gpu_memory, preference
):
    task_settings = {
        "vqa": {
            "query": "vision-language",
            "tags": ["vqa"],
            "intro": "VQA recommendations:",
        },
        "image_captioning": {
            "query": "image-captioning",
            "tags": ["image-captioning"],
            "intro": "Captioning models:",
        },
        "visual_language": {
            "query": "vision-language",
...
def process_chat_message(
    ...
    recommendation = get_model_recommendations(
        task_type, param_range_min, param_range_max, hardware_gpu_memory, preference
    )
