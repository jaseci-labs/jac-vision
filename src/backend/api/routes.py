...
from services.training import AVAILABLE_MODELS, train_model, train_model_with_goal, task_status, trained_models
...
async def train_with_goal(request: GoalTrainingRequest, background_tasks: BackgroundTasks):
    task_id = str(uuid.uuid4())
    try:
        background_tasks.add_task(train_model_with_goal, task_id, request.model_name, request.dataset_path, request.goal_type, request.target)
        return {"task_id": task_id, "status": "STARTED"}
...
def save_gguf_endpoint(request: GGUFSaveRequest):
    try:
        return save_gguf_model(task_id=request.task_id, output_dir=request.output_dir, quant_method=request.quant_method)
    except:
        raise HTTPException(status_code=404, detail="Model not found or training not completed")